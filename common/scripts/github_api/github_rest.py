...
def _mark_status(commit_sha, context, state, description, target_url=None):
  ...
  return session.post(url=_get_url('repos', 'statuses/{}'.format(commit_sha)),
                      json=body)


def mark_status_success(commit_sha,
                        status_name,
                        description,
                        check_res=True,
                        target_url=None):
  response = _mark_status(commit_sha, status_name, 'success', description,
                          target_url)
  if check_res:
    check_response(response)
  return response
...
def mark_status_failure(commit_sha,
                        status_name,
                        description,
                        check_res=True,
                        target_url=None):
  response = _mark_status(commit_sha, status_name, 'failure', description,
                          target_url)
  if check_res:
    check_response(response)
  return response
...
def create_pr(head_branch, base_branch):
  with open(
      os.path.join(os.path.dirname(__file__),
                   "pull_request_content_template")) as f:
    ...
    return session.post(url=_get_url('repos', 'pulls'), json=body)


def get_pull_request(branch_name: str):
  # List all open pull requests
  response = requests.get(url='{}/repos/{}/pulls?state=open'.format(
      API_URL, REPOS_NAME),
                          headers={"Authorization": "token {}".format(token)})
  data = response.json()
...
def get_approved_reviewers(pull_numbers):
  response = requests.get(
      url='{}/repos/{}/pulls/{}/reviews?per_page={}'.format(
          API_URL, REPOS_NAME, pull_numbers, 100),
      headers={"Authorization": "token {}".format(token)})
  data = response.json()
  approved_reviewers = []
